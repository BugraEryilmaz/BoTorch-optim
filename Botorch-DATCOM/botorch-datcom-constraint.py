...
seed_range = range(101,112) if args.grid else ([104] if args.transfer else [2])
Init_range = [50,2,5,10,20] if args.grid else ([5] if args.transfer else [20])
...
for seed in seed_range:
    for NUMBER_OF_INIT_POINTS in Init_range:
        try:
            ...
            new_df.to_csv(f'results_constrained{"_transfer" if TRANSFER else ""}_{NUMBER_OF_INIT_POINTS}_{seed}.csv')
