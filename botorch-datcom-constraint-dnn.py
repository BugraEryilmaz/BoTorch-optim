...
random.seed(223192)
np.random.seed(223192)
torch.manual_seed(223192)
...
# preds = dnn(np.asarray([array_from_parameters(datcom_exp.arms_by_name[item].parameters) for item in new_df['arm_name ']]))

# updated_obj = pd.Series([diff+dnn.forward(inps) for (diff, inps) in zip(new_df['objective'], new_df['parameters'])])
...
new_df.to_csv(f'results_constrained_dnn.csv')
